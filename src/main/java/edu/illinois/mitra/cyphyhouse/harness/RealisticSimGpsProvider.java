...
public class RealisticSimGpsProvider extends Observable implements SimGpsProvider {	
	...
	private Map<String, TrackedModel<ModelF110Car>> f110Cars;
	...
	private PositionList<ModelF110Car> f110car_positions;
	...
	public RealisticSimGpsProvider(SimulationEngine se, long period, double angleNoise, double posNoise) {
		...
		iRobots = new ConcurrentHashMap<>();
		f110Cars = new ConcurrentHashMap<>();
		quadcopters = new ConcurrentHashMap<>();
	...
	public synchronized void addRobot(TrackedRobot bot) {
		...
		else if(bot instanceof ModelF110Car) {
			synchronized (f110Cars) {
				f110Cars.put(((ModelF110Car)bot).name, new TrackedModel<>((ModelF110Car)bot));
			}
			f110car_positions.update((ModelF110Car)bot);
		}
		else{
	...
	public void setVelocity(String name, int fwd, int rad) {
		(iRobots.get(name).cur).vFwd = fwd;
		(iRobots.get(name).cur).vRad = rad;

	}

	@Override
	public void setVelocityForCar(String name, int fwd, int rad) {
		(f110Cars.get(name).cur).vFwd = fwd;
		(f110Cars.get(name).cur).vRad = rad;
	}
	@Override
	public void setControlInput(String name, double v_yaw, double pitch, double roll, double gaz) {
		/** TODO: replace with PID model here
		*/
		(quadcopters.get(name).cur).v_yawR = v_yaw;
		(quadcopters.get(name).cur).pitchR = pitch;
		(quadcopters.get(name).cur).rollR = roll;
		(quadcopters.get(name).cur).gazR = gaz;
	}
	
	@Override
	public synchronized void halt(String name) {
		setVelocity(name, 0, 0);
		setVelocityForCar(name, 0, 0);
	...
	@Override
	public PositionList<ModelF110Car> getF110CarPositions() {return f110car_positions;}
		...
		Thread posupdate = new Thread() {
			...
			public void run() {
				...
				while(true) {
					...
					synchronized (f110Cars) {
						for(TrackedModel<ModelF110Car> r : f110Cars.values()) {
							r.updatePos();
							receivers.get(r.getName()).receivePosition((r.cur.inMotion()));
						}
					}
	...
	public void notifyObservers(Object data) {
		...
		if(data == null) System.out.println("null nigga");
	...
	private class TrackedModel<T extends ItemPosition & TrackedRobot>{
		...
		public boolean checkCollision(Point3d bot) {
			...
			else if (cur instanceof ModelF110Car) {
				myRadius = ((ModelF110Car) cur).radius;
			}
			else{
			...
			for(ModelF110Car current : f110car_positions.getList()) {
				if(!current.name.equals(cur.name)) {
					if (bot.distanceTo(current) <= myRadius + current.radius) {
						//update sensors for both robots
						current.collision(cur);
