...
public class DrawPanel extends ZoomablePanel
{
	...
	private void drawRobot(Graphics2D g, RobotData rd, boolean drawId){
		...
		if(rd.name.contains(settings.QUADCOPTER_NAME)){
			...
			g.drawOval(rd.x - radius, rd.y - radius, radius*2, radius*2);
		else if(rd.name.contains(settings.CAR_NAME)){
			Rectangle body = new Rectangle(rd.x, rd.y - 20, 120, 20);
			Ellipse2D.Double frontTire = new Ellipse2D.Double(rd.x + 20, rd.y + 20,
					20, 20);
			Ellipse2D.Double rearTire = new Ellipse2D.Double(rd.x + 80, rd.y + 40,
					20, 20);
			// The bottom of the front windsheild
			Point2D.Double p1 = new Point2D.Double(rd.x + 20, rd.y + 20);
			// The Front of the roof
			Point2D.Double p2 = new Point2D.Double(rd.x + 40, rd.y);
			Point2D.Double p3 = new Point2D.Double(rd.x + 80, rd.y);
			Point2D.Double p4 = new Point2D.Double(rd.x + 100, rd.y + 20);
