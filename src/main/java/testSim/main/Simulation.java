...
public class Simulation {
	...
	public Simulation(Class<? extends LogicThread> app, final SimSettings settings) {
		if(settings.N_IROBOTS + settings.N_QUADCOPTERS + settings.N_CARS <= 0)
		...
		for(int j = 0; j < settings.N_QUADCOPTERS; j++) {
			...
			System.out.println("quadxx");
			participants.put(settings.QUADCOPTER_NAME + j, "192.168.0." + (j+settings.N_IROBOTS));
		}
		for(int k = 0; k < settings.N_CARS; k++) {
			System.out.println("carxx");
			participants.put(settings.CAR_NAME + k, "192.168.0." + (k+settings.N_IROBOTS));
		}
		...
		for(int i = 0; i < settings.N_CARS; i++) {
			ModelF110Car initialPosition = null;
			String botName = settings.CAR_NAME + i;
			ItemPosition initialPos = t_initialPositions.getPosition(botName);
			if(initialPos != null){
				initialPosition = new ModelF110Car(initialPos);
			}
			// If no initial position was supplied, randomly generate one
			if(initialPosition == null) {
				//	System.out.println("null position in list");
				...
				while(retries++ < 10000 && (!acceptableStart(initialPosition) || !valid))
				{
					initialPosition = new ModelF110Car(botName, rand.nextInt(settings.GRID_XSIZE), rand.nextInt(settings.GRID_YSIZE), rand.nextInt(360));
					if(list != null){
						valid = (list.validstarts(initialPosition, initialPosition.radius));
					}
				}
				...
				public void update(Observable o, Object arg) {
					...
					if(targetList.size() >0){
						for(int i = 0; i < targetList.size(); i++){
							...
							else if(targetList.get(i) instanceof ModelF110Car) {
								ModelF110Car ip = (ModelF110Car) targetList.get(i);
		...
		Observer globalLogger = new Observer() {
			...
			public void update(Observable o, Object arg) {
                ...
                for(ItemPosition ip : pos) {
                    ...
                    else if(ip instanceof ModelF110Car) {
						ModelF110Car m = (ModelF110Car) ip;
						RobotData nextBot = new RobotData(m.name, m.x, m.y, m.angle, ip.receivedTime);
						nextBot.radius = settings.BOT_RADIUS;
						rd.add(nextBot);
					}
	...
	public void start() {
		...
		try {
			...
			if(settings.TIMEOUT > 0) {
                ...
                System.out.println("do you go here pt2");
